...
public class BlogService {
    ...
    public void deleteById(Integer id) {
        ...
        if (blog != null) {
            Account currentUser = TokenUtils.getCurrentUser();
            if (RoleEnum.USER.name().equals(currentUser.getRole()) && !currentUser.getId().equals(blog.getUserId())) {
    ...
    public void updateById(Blog blog) {
        ...
        if (dbBlog != null) {
            Account currentUser = TokenUtils.getCurrentUser();
            if (RoleEnum.USER.name().equals(currentUser.getRole()) && !currentUser.getId().equals(dbBlog.getUserId())) {
                throw new CustomException(ResultCodeEnum.NO_AUTH);
            }
        }
        blog.setContent(HtmlUtils.sanitize(blog.getContent()));  // 富文本消毒，防 XSS
    ...
    public Blog selectById(Integer id) {
        Blog blog = blogMapper.selectById(id);
        // 作者信息可能不存在（如管理员创建的博客），需要判空
        User user = blog.getUserId() == null ? null : userService.selectById(blog.getUserId());
        if (user != null) {
     ...
     * 关注的人发布的博客（关注流）
     */
    public PageInfo<Blog> selectFollow(Blog blog, Integer pageNum, Integer pageSize) {
        Account currentUser = TokenUtils.getCurrentUser();
        blog.setUserId(currentUser.getId());
        PageHelper.startPage(pageNum, pageSize);
        List<Blog> list = blogMapper.selectFollow(blog);
        PageInfo<Blog> pageInfo = PageInfo.of(list);
        List<Blog> blogList = pageInfo.getList();
        for (Blog b : blogList) {
            int likesCount = likesService.selectByFidAndModule(b.getId(), LikesModuleEnum.BLOG.getValue());
