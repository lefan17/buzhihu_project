<template>
  <div class="main-content">
    <div class="card" style="margin-bottom: 10px">
      <div style="display: flex; grid-gap: 20px">
        ...
        <div style="flex: 1;">
          ...
          <div style="color: #666; font-size: 13px; line-height: 22px; text-align: justify; margin-bottom: 10px">
            {{ activity.descr }}
          </div>
          <div style="color: #666; margin-bottom: 10px"><i class="el-icon-date"></i> 时间 <span
              style="margin-left: 5px">{{ activity.start }} ~ {{ activity.end }}</span></div>
          <div style="color: #666; margin-bottom: 10px"><i class="el-icon-location"></i> 地址
            <a style="color: #2a60c9; margin-left: 5px" :href="activity.address"
               v-if="activity.form === '线上'">{{ activity.address }}</a>
          ...
          <div style="display: flex; align-items: center">
            ...
            <div style="text-align: right; flex: 1">
              ...
              <el-button type="warning" @click="collect" v-if="activity.isCollect">已收藏 {{
          ...
          <div style="text-align: right; margin-top: 10px">
            <span style="font-size: 14px; cursor: pointer; color: #999" @click="openReport">举报该活动</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="card" style="margin-bottom: 10px">
      <div
          style="font-weight: bold; font-size: 24px; margin-bottom: 20px; padding-left: 10px; border-left: 5px solid #2a60c9">
        活动详情
      </div>
      <div class="w-e-text">
...
export default {
  ...
  methods: {
    ...
    submitReport() {
      ...
      this.$request.post('/report/add', {fid: Number(this.activityId), module: '活动', reason: reason}).then(res => {
        if (res.code === '200') {
    ...
    cancel() {   // 单个删除
      this.$confirm('您确定取消报名吗？', '确认取消', {type: "warning"}).then(response => {
        this.$request.delete('/activitySign/deleteByActivity?activityId=' + this.activityId).then(res => {
